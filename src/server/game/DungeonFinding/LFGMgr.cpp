...
namespace
{
    // Virtual "Random Dungeon" queue ids for pure 3.3.5 Wrath content (Random Normal/Heroic 258-262).
    // Titan Rune Alpha/Beta/Gamma are intentionally NOT listed here - see g_DisabledDungeons below.
    static constexpr std::array<uint16, 5> g_RandomDungeons = { 258, 259, 260, 261, 262 };

    // Dungeons that must never be queueable on this server, no matter what the 3.4.3 client's
    // LFGDungeons.db2 (or a future lfg_dungeon_template row) says. This is a pure 3.3.5 Blizzlike
    // server; the Wrath Classic Heroic+ Titan Rune dungeons are a modern addition we don't want.
    //   2485 = Titan Rune: Alpha Complex (Heroic+)
    //   2470 = Titan Rune: Beta Complex (Heroic+)
    //   2447 = Titan Rune: Gamma Complex (Heroic+)
    static constexpr std::array<uint16, 3> g_DisabledDungeons = { 2485, 2470, 2447 };

    bool IsDisabledDungeon(uint32 dungeonId)
    {
        return std::find(g_DisabledDungeons.begin(), g_DisabledDungeons.end(), dungeonId) != g_DisabledDungeons.end();
    }

    constexpr uint32 SOUND_PVP_ENTER_QUEUE = 8462;
    constexpr uint32 SOUND_LFG_DENIED = 17341;
}
...
namespace lfg
{
    ...
    */
    void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons)
    {
        ...
        // Alistar: disable role filtering for now.
#if 0
    // Sanitize input roles
        ...
        if (joinData.result == LFG_JOIN_OK)
        {
            ...
            {
                // Pure 3.3.5 Blizzlike server: permanently reject Titan Rune Alpha/Beta/Gamma,
                // regardless of what the client sent or what the DB2/DB currently says about them.
                // Checks both the umbrella "Random ..." queue id itself and, via the dungeon's own
                // GroupID, any individual heroic+ variant dungeon that belongs to one of those groups.
                {
                    LFGDungeonData const* candidate = GetLFGDungeon(*it);
                    if (IsDisabledDungeon(*it) || (candidate && IsDisabledDungeon(candidate->group)))
                    {
        ...
        if (grp)                                               // Begin rolecheck
        {
            if (isContinue)
            {
                // Requeueing an in-progress LFG group after someone left/was kicked.
                // Real WotLK RDF does NOT make the remaining members reconfirm their roles -
                // the tool already remembers them, and only the leader has to accept the
                // "find a replacement" prompt. So skip the interactive role check entirely
                // and drop the (still intact) group straight into the queue, high priority,
                // reusing everyone's previously selected role.
                ...
                for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
                {
                    if (Player* plrg = itr->GetSource())
                    {
                        ...
                        uint8 pRoles = (pguid == guid) ? roles : GetRoles(pguid);
                        SetTicket(pguid, ticket);
                        SetRoles(pguid, pRoles);
                        SetState(pguid, LFG_STATE_QUEUED);
                        rolesMap[pguid] = pRoles;
                        plrg->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, dungeons), true);
                        if (!debugNames.empty())
                            debugNames.append(", ");
                ...
                LFGQueue& queue = GetQueue(gguid);
                // reAdd = true: place this group at the FRONT of the queue. Real WotLK RDF gives
                // an incomplete, already-active dungeon group priority over brand new groups when
                // looking for a replacement.
                queue.AddQueueData(gguid, GameTime::GetGameTime(), dungeons, rolesMap, true);
            }
            else
            {
        ...
        player->PlayDirectSound(SOUND_PVP_ENTER_QUEUE, player);
    ...
    */
    void LFGMgr::LeaveLfg(ObjectGuid guid, bool disconnected)
    {
            ...
            if (!gguid.IsEmpty())
            {
                ...
                for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
                {
                    ...
                    if (Player* player = ObjectAccessor::FindPlayer(*it))
                        player->PlayDirectSound(SOUND_LFG_DENIED, player);
                }
            }
            else
            {
        ...
        case LFG_STATE_RAIDBROWSER:
            SetState(guid, LFG_STATE_NONE); // Triggers Ticket Wipe
            SendLfgUpdateStatus(guid, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE, LFG_STATE_NONE, {}), false);
            break;
        case LFG_STATE_DUNGEON:
        case LFG_STATE_FINISHED_DUNGEON:
            if (guid != gguid && !disconnected) // Single Player Leaves
            {
                SetState(guid, LFG_STATE_NONE); // Triggers Ticket Wipe
                SendLfgUpdateStatus(guid, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE, LFG_STATE_NONE, {}), false);
            }
            else if (guid == gguid) // Group is disbanded
            {
                SetState(gguid, LFG_STATE_NONE);
                GuidSet const& players = GetPlayers(gguid);
                for (ObjectGuid pguid : players)
                {
                    SetState(pguid, LFG_STATE_NONE); // Triggers Ticket Wipe
                    SendLfgUpdateStatus(pguid, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE, LFG_STATE_NONE, {}), false);
                }
            }
            break;
        }
    ...
    */
    void LFGMgr::TeleportPlayer(Player* player, bool out, bool fromOpcode /*= false*/)
    {
        ...
        // else: player is already on the dungeon map - nothing to do, this is success, not an
        // error. (A stray "else error = LFG_TELEPORT_RESULT_NO_RETURN_LOCATION;" used to live here,
        // which is why players already inside the instance got "You do not have a valid teleport
        // location" whenever the client sent CMSG_DF_TELEPORT for them, e.g. after a proposal
        // update/replacement search told every party member's client to show an "Enter Dungeon"
        // button, including the ones who were already there.)
    ...
    */
    ...
    LfgLockMap LFGMgr::GetLockedDungeons(ObjectGuid guid)
    {
        ...
        const auto softLock = [](uint32 lockStatus)
            {
                switch (lockStatus)
                {
        ...
        // alistar: go through random dungeons first
        for (const uint16 dungeon : g_RandomDungeons)
        {
            if (LFGDungeonsEntry const* dungeonEntry = sLFGDungeonsStore.LookupEntry(dungeon))
            {
                uint32 lockStatus = 0;

                if (dungeonEntry->ExpansionLevel > expansion)
                    lockStatus = LFG_LOCKSTATUS_INSUFFICIENT_EXPANSION;

                if (dungeonEntry->MinLevel > level || level > dungeonEntry->MaxLevel)
                    lockStatus = LFG_LOCKSTATUS_TOO_LOW_LEVEL;

                if (IsDisabledDungeon(dungeon))
                    lockStatus = LFG_LOCKSTATUS_NOT_IN_SEASON;

                if (lockStatus)
                    lock[dungeonEntry->Entry()] = LfgLockInfoData(lockStatus, dungeonEntry->MentorItemLevel, player->GetAverageItemLevel(), softLock(lockStatus));
            }
        }
        ...
        for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
        {
            ...
            const uint32 lockStatus = [&]() -> uint32
                {
                    if (denyJoin)
                    ...
                    if (IsDisabledDungeon(dungeon->id) || IsDisabledDungeon(dungeon->group))
                        return LFG_LOCKSTATUS_NOT_IN_SEASON;
            ...
            if (lockStatus)
                lock[dungeon->Entry()] = LfgLockInfoData(lockStatus, dungeon->requiredItemLevel, player->GetAverageItemLevel(), softLock(lockStatus));
        }

        // Defense in depth: GetDungeonsByRandom(0) above only walks "standalone" dungeons
        // (GroupID == 0). If the specific heroic+ variants that make up a Titan Rune complex are
        // grouped under GroupID 2485/2470/2447, they'd never be visited by that loop and would
        // keep showing up, unlocked, in the client's "Specific Dungeon" browse list. Walk the full
        // dungeon store once more and lock (and thus hide - see softLock above) anything tied to a
        // disabled group that we haven't already covered.
        for (LFGDungeonContainer::const_iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
        {
            ...
            if (lock.find(dungeon.Entry()) != lock.end())
                continue;

            if (IsDisabledDungeon(dungeon.id) || IsDisabledDungeon(dungeon.group))
                lock[dungeon.Entry()] = LfgLockInfoData(LFG_LOCKSTATUS_NOT_IN_SEASON, dungeon.requiredItemLevel, player->GetAverageItemLevel(), softLock(LFG_LOCKSTATUS_NOT_IN_SEASON));
        }
    ...
    {
        // alistar: return random dungeon
        if (std::find(std::begin(g_RandomDungeons), std::end(g_RandomDungeons), id) != std::end(g_RandomDungeons))
            if (LFGDungeonsEntry const* dungeonEntry = sLFGDungeonsStore.LookupEntry(id))
                return dungeonEntry->Entry();
    ...
    LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(uint8 level, uint8 expansion)
    {
        ...
        for (lfg::LFGDungeonContainer::const_iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
        {
            ...
            if (IsDisabledDungeon(dungeon.id))
                continue;
        ...
        for (const uint16 dungeon : g_RandomDungeons)
        {
            if (LFGDungeonsEntry const* dungeonEntry = sLFGDungeonsStore.LookupEntry(dungeon))
            {
                ...
                randomDungeons.insert(dungeonEntry->Entry());
            }
        }
