...
namespace
{
    static constexpr std::array<uint16, 8> g_RandomDungeons = { 258, 259, 260, 261, 262, 2447, 2470, 2485 };
        ...
        case LFG_STATE_QUEUED:
            if (!gguid.IsEmpty())
            {
                ...
                for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
                {
                    ...
                    // alistar: for some reason for join/leave df they didn't play this sound client side. (Group)
            ...
            else
            {
                ...
                // alistar: for some reason for join/leave df they didn't play this sound client side. (Solo)
...
LfgLockMap LFGMgr::GetLockedDungeons(ObjectGuid guid)
{
    ...
    for (const uint16 dungeon : g_RandomDungeons)
    {
        if (LFGDungeonsEntry const* dungeonEntry = sLFGDungeonsStore.LookupEntry(dungeon))
        {
            ...
            // alistar: disable titan rune dungeons for now
            if (dungeon > 2000)
